...
function resolutionType(refractClasses) {
  console.log(`\nrefract classes:\n${JSON.stringify(refractClasses,' ', 2)}`)
...
function resolutionRange(endingsIndices, sourceMapAttribute) {
  ...
  console.log('\nsource map attribute:\n' + JSON.stringify(sourceMapAttribute.toValue(), ' ', 2) + '\n')


  // Consider only first Source Map element
  let sourceMapElement = sourceMapAttribute.getIndex(0) //minim.fromRefract(sourceMap[0])
  console.log('\nsource map element:\n' + JSON.stringify(sourceMapElement.toValue(), ' ', 2) + '\n')

  let indices = sourceMapElement.toValue().content
...
function createResolution(endingsIndices, filePath, annotation) {
  ...
  return {
    ...
    range: resolutionRange(endingsIndices, annotation.attributes.get('sourceMap'))
  }
}
